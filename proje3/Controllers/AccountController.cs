...
namespace proje3.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            using (var context = new ProjeContext())
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Succeeded)
                    {
                        result = await UserManager.AddToRoleAsync(user.Id, MyConstants.RoleUser);//role ekleme
                        //await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);//giriş yapıyor yeni kullanıcyla bunu istemiyoruz
        ...
        private ActionResult RedirectToLocal(string returnUrl)
        {
            ...
            return RedirectToAction("Sorgu", "Home");
        }
